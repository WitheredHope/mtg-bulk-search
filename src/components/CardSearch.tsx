...
const CardSearch = () => {
  ...
  const handleSaveList = async () => {
    ...
    try {
      const cardEntries = cardList
        .split('\n')
        .map(line => line.trim())
        .filter(line => line.length > 0)
        .map(entry => {
          const match = entry.match(/^(\d+)(?:\s*x\s*)?\s*(.+)$/);
          if (match) {
            return {
              name: match[2].trim(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Parse quantities from card names
      const cardNames = cardEntries.map(entry => {
        // Match patterns like "2 Card Name" or "2x Card Name"
        const match = entry.match(/^(\d+)(?:\s*x\s*)?\s*(.+)$/);
        if (match) {
          return {
            name: match[2].trim(),
      ...
      const { foundCards, unfoundCards } = await searchCards(cardNames.map(c => c.name));
      
      // Add quantities to found cards
      const foundCardsWithQuantities = foundCards.map(card => {
        const entry = cardNames.find(c => c.name.toLowerCase() === card.name.toLowerCase());
        return {
          ...card,
